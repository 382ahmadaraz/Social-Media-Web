...
export default function Ptable(detail) {
  return (
    ...
    <div className="flex justify-evenly lg:justify-between flex-wrap">
      {tableData.map((index) => {
        return (
          <div key={index.btn} className="border-2 p-2 mb-3 md:p-4 lg:w-[24%] rounded-xl h-fit text-sm leading-9 2xl:text-lg 2xl:leading-loose ">
              ...
              return (
                <div key={id}>
                  <p className="flex items-center text-slate-500">
                    <img src={Tick} alt="tick mark" className="w-[13px] h-[10px] mr-2" />
                    {detail}
                  </p>
                </div>
              );
